...
/// Abstraction over the libhowl C ABI. Production impl is `LibhowlEngine`;
/// tests inject a fake.
public protocol CoreEngine: Sendable {
    ...
    /// Returns the JSON array of session manifests from libhowl, or nil
    ...
    /// Returns the JSON array of presets (bundled + user) from libhowl,
    ...
    /// howl_replay (or {"error": "..."} envelope on top-level failure),
    /// or nil if the engine is not initialized.
